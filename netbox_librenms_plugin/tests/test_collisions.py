...
def test_promote_to_host_without_existing_device_does_not_collide():
    """After the PR the ``existing_device_pk`` fallback was removed from
    _candidate_pks_for_row.  A promote_to_host dict that only carries
    ``existing_device_pk`` (and no ``existing_device`` object) must now
    produce ZERO candidates — so two such rows targeting the same pk id
    are NOT reported as a collision."""
    devices = [
        _row(3000, "row-a", {"promote_to_host": {"existing_device_pk": 55, "existing_libre_id": 10}}),
        _row(3001, "row-b", {"promote_to_host": {"existing_device_pk": 55, "existing_libre_id": 11}}),
    ]
    # Without existing_device objects the candidates list is empty, so no collision.
    assert detect_bulk_collisions(devices) == []


def test_promote_to_host_with_existing_device_still_collides():
    """Sanity check: when promote_to_host carries an existing_device object
    (the current format after the PR), collisions are still detected."""
    nb_device = Device(pk=55, name="target")
    devices = [
        _row(3100, "row-a", {"promote_to_host": {"existing_device": nb_device, "existing_libre_id": 10}}),
        _row(3101, "row-b", {"promote_to_host": {"existing_device": nb_device, "existing_libre_id": 11}}),
    ]
    groups = detect_bulk_collisions(devices)
    assert len(groups) == 1
    assert groups[0]["nb_device_pk"] == 55
    assert {r["device_id"] for r in groups[0]["librenms_rows"]} == {3100, 3101}


def test_collision_group_rows_sorted_by_device_id():
    """librenms_rows within a collision group must be sorted by device_id
    for stable rendering in the template."""
    nb_device = Device(pk=66, name="sorted-test")
    devices = [
        _row(5002, "z", {"existing_device": nb_device}),
        _row(5000, "a", {"existing_device": nb_device}),
        _row(5001, "m", {"existing_device": nb_device}),
    ]
    groups = detect_bulk_collisions(devices)
    assert len(groups) == 1
    ids = [r["device_id"] for r in groups[0]["librenms_rows"]]
