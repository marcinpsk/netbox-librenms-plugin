...
class TestRefreshLibreNMSLinkage:
    ...
    def test_string_digit_scanned_id_matches_int_host_id(self):
        """coerce_librenms_id normalises string-digit device_ids from the API
        (e.g. '42') so they compare equal to int host_ids stored in the CF."""
        v = self._call(match_type="librenms_id", scanned_id="42", host_id=42, oob_id=None)
        assert v["existing_match_type"] == "librenms_id"

    def test_string_digit_scanned_id_matches_oob_id(self):
        """String-digit device_id must also match against the OOB id."""
        v = self._call(match_type="librenms_id", scanned_id="77", host_id=None, oob_id=77)
        assert v["existing_match_type"] == "librenms_oob"

    def test_existing_librenms_link_always_refreshed(self):
        """existing_librenms_link must be populated regardless of match type."""
        v = self._call(match_type="hostname", scanned_id=10, host_id=None, oob_id=None)
        assert "existing_librenms_link" in v
        assert v["existing_librenms_link"] == {"host_id": None, "oob_id": None, "oob_type": None}


class TestClearExistingMatchDerivedFields:
    """Unit tests for _clear_existing_match_derived_fields (added in this PR).

    This helper resets the derived fields that are populated from an existing
    device match so that stale serial/OOB/merge/promote actions don't persist
    after the match is dropped (device deleted or link removed since caching).
    """

    def _call(self, validation: dict) -> dict:
        from netbox_librenms_plugin.import_utils import bulk_import

        bulk_import._clear_existing_match_derived_fields(validation)
        return validation

    def _full_validation(self):
        """Return a validation dict fully populated with the fields this helper clears."""
        return {
            "serial_action": "oob_candidate",
            "oob_candidate": {"device": MagicMock(), "type": "idrac", "version": None, "ip": "10.0.0.1"},
            "serial_role_choice_available": True,
            "promote_to_host": {"existing_libre_id": 5, "existing_oob_type": "idrac"},
            "merge_candidates": {"host_named": {"pk": 1}, "oob_named": {"pk": 2}},
            "existing_device": MagicMock(),
        }

    def test_clears_serial_action(self):
        v = self._call(self._full_validation())
        assert v["serial_action"] is None

    def test_clears_oob_candidate(self):
        v = self._call(self._full_validation())
        assert v["oob_candidate"] is None

    def test_clears_serial_role_choice_available(self):
        v = self._call(self._full_validation())
        assert v["serial_role_choice_available"] is False

    def test_removes_promote_to_host_key(self):
        """promote_to_host must be removed entirely ('absent otherwise' contract),
        not set to None, so downstream code that checks 'if promote_to_host in result'
        correctly sees the field as absent."""
        v = self._call(self._full_validation())
        assert "promote_to_host" not in v

    def test_removes_merge_candidates_key(self):
        """merge_candidates must also be removed entirely."""
        v = self._call(self._full_validation())
        assert "merge_candidates" not in v

    def test_noop_on_already_cleared_validation(self):
        """Calling the helper on a validation that already lacks the optional keys
        must not raise (pop on absent key should not crash)."""
        v = {
            "serial_action": None,
            "oob_candidate": None,
            "serial_role_choice_available": False,
            # promote_to_host and merge_candidates intentionally absent
        }
        result = self._call(v)
        assert result["serial_action"] is None
        assert "promote_to_host" not in result
        assert "merge_candidates" not in result

    def test_does_not_touch_unrelated_fields(self):
        """Fields outside the helper's scope must remain unchanged."""
        v = self._full_validation()
        sentinel = object()
        v["existing_device"] = sentinel
        v["can_import"] = True
        v["is_ready"] = True
        self._call(v)
        assert v["existing_device"] is sentinel
        assert v["can_import"] is True
