...
// Host/OOB interface name collision: the rename the operator confirms before anything is written.
// The button only opens the modal (rendered by _render_name_collision_pill in
// tables/interfaces.py); the POST happens on Rename, so a collision is never resolved by a click
// alone. Defaults to renaming the OOB side, which is the side the host took the name from.
(function () {
    let pendingCollision = null;

    const modalElement = () => document.getElementById('nameCollisionModal');

    const showCollisionError = (message) => {
        const box = document.getElementById('nameCollisionError');
        if (!box) return;
    ...
    document.addEventListener('click', function (e) {
        const btn = e.target.closest('.name-collision-btn');
        if (!btn) return;
        e.preventDefault();

        const element = modalElement();
        // No modal on the page (a partial render) → say so rather than silently doing nothing.
        if (!element) {
            btn.innerHTML = '<i class="mdi mdi-alert text-danger"></i>';
            btn.title = 'The rename dialog is unavailable. Refresh the page and try again.';
            return;
        }

        pendingCollision = {
            url: btn.dataset.resolveUrl || '',
            portId: btn.dataset.portId || '',
            name: btn.dataset.interface || '',
            proposedName: btn.dataset.proposedName || '',
            button: btn,
        };

        document.getElementById('nameCollisionName').textContent = pendingCollision.name;
        const oobRadio = document.getElementById('nameCollisionSideOob');
        if (oobRadio) oobRadio.checked = true;
        const input = document.getElementById('nameCollisionNewName');
        if (input) input.value = pendingCollision.proposedName;
        showCollisionError('');
        // NetBox bundles Bootstrap as an ES module, so there is no window.bootstrap to call.
        showModal(element);
    });

    // Switching sides re-proposes: renaming the host frees the contested name, so the OOB port
    // takes it as-is and the host needs a name of its own.
    document.addEventListener('change', function (e) {
        const radio = e.target.closest('input[name="name_collision_side"]');
        if (!radio || !pendingCollision) return;
        const input = document.getElementById('nameCollisionNewName');
        if (!input) return;
        input.value = radio.value === 'host' ? `${pendingCollision.name}-host` : pendingCollision.proposedName;
        showCollisionError('');
    });

    document.addEventListener('click', function (e) {
        const applyButton = e.target.closest('#nameCollisionApply');
        if (!applyButton || !pendingCollision) return;
        e.preventDefault();

        const csrf = getCsrfToken();
        if (!csrf) {
            showCollisionError('CSRF token not found. Refresh the page and try again.');
            return;
        }
        const newName = (document.getElementById('nameCollisionNewName')?.value || '').trim();
        if (!newName) {
            showCollisionError('Enter a new interface name.');
            return;
        }
        const side = document.querySelector('input[name="name_collision_side"]:checked')?.value || 'oob';

        const body = new URLSearchParams({
            csrfmiddlewaretoken: csrf,
            port_id: pendingCollision.portId,
            side: side,
            new_name: newName,
            interface_name_field: document.querySelector('input[name="interface_name_field"]:checked')?.value || '',
            server_key: document.querySelector('[name=server_key]')?.value || '',
        });

        applyButton.disabled = true;
        showCollisionError('');

        fetch(pendingCollision.url, {
            method: 'POST',
            headers: { 'X-CSRFToken': csrf, 'Content-Type': 'application/x-www-form-urlencoded' },
            body: body.toString(),
        })
            .then(function (r) {
                if (!r.ok) {
                    return fetchErrorMessage(r).then(function (msg) { throw new Error(msg); });
                }
                const transition = r.headers.get('X-LibreNMS-Cache-Transition');
                if (transition) {
                    try {
            ...
            .then(function (data) {
                ...
                if (data.status !== 'success') {
                    showCollisionError(data.error || 'The rename failed.');
                    return;
                }
                hideModal(modalElement());
                // Both outcomes change NetBox interface names, which the whole table compares
                // against. Reload rather than patch one cell and leave the rest stale.
                window.location.reload();
            })
            .catch(function (err) {
                applyButton.disabled = false;
                showCollisionError(err.message || 'The request failed.');
            });
    });
})();
